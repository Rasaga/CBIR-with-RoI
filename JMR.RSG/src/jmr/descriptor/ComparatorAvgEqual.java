...
/**
 * Clase para la creación del comparador Igual Media.
 * 
 * @author Ramón Sánchez García (chentaco@correo.ugr.es)
 */
public class ComparatorAvgEqual implements Comparator<SegmentedDescriptor, Double>, Serializable {
    
    boolean position;
    
    /**
     * Constructor de la clase de comparador medio usando igualdad.
     * @param position variable booleana para tomar en cuenta la posición
     * de la región o no.
     */       
    public ComparatorAvgEqual(boolean position){
        super();
     ...
     * descriptores segmentados en función de la media y del numero de regiones.
    ...
    public Double apply(SegmentedDescriptor t, SegmentedDescriptor u) {
        Double resultado;
        if (t.size() == u.size()) {
            ComparatorAvg p = new ComparatorAvg(position);
